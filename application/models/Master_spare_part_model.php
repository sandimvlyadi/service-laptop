...
class Master_spare_part_model extends CI_Model {

	function _get($data = array())
    {
    	$q = "SELECT a.* FROM `spare_part` a ";

        if ($data['search']['value'] && !isset($data['all'])) {
        	$s = $this->db->escape_str($data['search']['value']);
            $q .= "WHERE (a.`nama_spare_part` LIKE '%". $s ."%' OR a.`keterangan` LIKE '%". $s ."%') AND a.`deleted_at` IS NULL ";
        } else{
		...
		if ($id == 0) {
			$q =    "INSERT INTO
                    ...
                    VALUES
                        (
                            NOW(),
                            '". $this->db->escape_str($f['nama_spare_part']) ."',
            ...
            $q =    "UPDATE
                    ...
                    SET
                        `modified_at` = NOW(),
                        `nama_spare_part` = '". $this->db->escape_str($f['nama_spare_part']) ."',
	...
	function delete($data = array())
	{
		...
		$d = $data['postData'];
		$id = $d['id'];
		$q = "UPDATE `spare_part` SET `deleted_at` = NOW() WHERE `id` = '". $this->db->escape_str($id) ."';";
		if ($this->db->simple_query($q)) {
			$result['result'] = true;
    ...
    function select($id = 0)
    {
        ...
        $q = "";
        if ($id == 0) {
            $q = "SELECT * FROM `spare_part` WHERE `deleted_at` IS NULL;";
        } else{
            $q = "SELECT * FROM `spare_part` WHERE `id` = '". $this->db->escape_str($id) ."' AND `deleted_at` IS NULL;";
        }
        $r = $this->db->query($q)->result_array();
        if (count($r) > 0) {
            $result['result'] = true;
