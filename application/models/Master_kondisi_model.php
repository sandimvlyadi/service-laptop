...
class Master_kondisi_model extends CI_Model {

	function _get($data = array())
    {
    	$q = "SELECT a.* FROM `kondisi` a ";

        if ($data['search']['value'] && !isset($data['all'])) {
        	$s = $this->db->escape_str($data['search']['value']);
            $q .= "WHERE (a.`nama_kondisi` LIKE '%". $s ."%' OR a.`keterangan` LIKE '%". $s ."%') AND a.`deleted_at` IS NULL ";
        } else{
    ...
    function edit($id = 0)
    {
        $result = array(
            'result'    => false,
            'msg'       => 'Data kondisi tidak ditemukan.'
        );
		...
		if ($id == 0) {
			$q =    "INSERT INTO
                    ...
                    VALUES
                        (
                            NOW(),
                            '". $this->db->escape_str($f['nama_kondisi']) ."',
            ...
            $q =    "UPDATE
                    ...
                    SET
                        `modified_at` = NOW(),
                        `nama_kondisi` = '". $this->db->escape_str($f['nama_kondisi']) ."',
	...
	function delete($data = array())
	{
		...
		$d = $data['postData'];
		$id = $d['id'];
		$q = "UPDATE `kondisi` SET `deleted_at` = NOW() WHERE `id` = '". $this->db->escape_str($id) ."';";
		if ($this->db->simple_query($q)) {
			$result['result'] = true;
    ...
    function select($id = 0)
    {
        ...
        $q = "";
        if ($id == 0) {
            $q = "SELECT * FROM `kondisi` WHERE `deleted_at` IS NULL;";
        } else{
            $q = "SELECT * FROM `kondisi` WHERE `id` = '". $this->db->escape_str($id) ."' AND `deleted_at` IS NULL;";
        }
        $r = $this->db->query($q)->result_array();
        if (count($r) > 0) {
            $result['result'] = true;
