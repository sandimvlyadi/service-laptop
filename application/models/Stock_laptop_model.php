...
class Stock_laptop_model extends CI_Model {
    ...
    {
    	$q = "SELECT a.*, b.`nama_kondisi`, c.`status` FROM `stock_laptop` a LEFT JOIN `kondisi` b ON a.`id_kondisi` = b.`id` LEFT JOIN `status` c ON a.`id_status` = c.`id` ";

        if ($data['search']['value'] && !isset($data['all'])) {
        	$s = $this->db->escape_str($data['search']['value']);
            $q .= "WHERE (b.`nama_kondisi` LIKE '%". $s ."%' OR c.`status` LIKE '%". $s ."%' OR a.`merk_serie` LIKE '%". $s ."%' OR a.`spesifikasi` LIKE '%". $s ."%' OR a.`persentase_kondisi` LIKE '%". $s ."%' OR a.`harga` LIKE '%". $s ."%' OR a.`keterangan` LIKE '%". $s ."%') AND a.`deleted_at` IS NULL ";
        } else{
