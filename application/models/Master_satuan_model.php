...
class Master_satuan_model extends CI_Model {
    ...
    {
    	$q = "SELECT a.* FROM `satuan` a ";

        if ($data['search']['value'] && !isset($data['all'])) {
        	$s = $this->db->escape_str($data['search']['value']);
            $q .= "WHERE (a.`kode_satuan` LIKE '%". $s ."%' OR a.`nama_satuan` LIKE '%". $s ."%' OR a.`keterangan` LIKE '%". $s ."%') AND a.`deleted_at` IS NULL ";
        } else{
    ...
    function edit($id = 0)
    {
        $result = array(
            'result'    => false,
            'msg'       => 'Data satuan tidak ditemukan.'
        );
		...
		if ($id == 0) {
			$q =    "INSERT INTO
                    ...
                    VALUES
                        (
                            NOW(),
                            '". $this->db->escape_str($f['kode_satuan']) ."',
                            '". $this->db->escape_str($f['nama_satuan']) ."',
            ...
            $q =    "UPDATE
                    ...
                    SET
                        `modified_at` = NOW(),
                        `kode_satuan` = '". $this->db->escape_str($f['kode_satuan']) ."',
                        `nama_satuan` = '". $this->db->escape_str($f['nama_satuan']) ."',
	...
	function delete($data = array())
	{
		...
		$d = $data['postData'];
		$id = $d['id'];
		$q = "UPDATE `satuan` SET `deleted_at` = NOW() WHERE `id` = '". $this->db->escape_str($id) ."';";
		if ($this->db->simple_query($q)) {
			$result['result'] = true;
    ...
    function select($id = 0)
    {
        ...
        $q = "";
        if ($id == 0) {
            $q = "SELECT * FROM `satuan` WHERE `deleted_at` IS NULL;";
        } else{
            $q = "SELECT * FROM `satuan` WHERE `id` = '". $this->db->escape_str($id) ."' AND `deleted_at` IS NULL;";
        }
        $r = $this->db->query($q)->result_array();
        if (count($r) > 0) {
            $result['result'] = true;
