...
class Master_status extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('master_status_model', 'model');
	...
	public function service($id = 0)
	{
		...
		$response = $this->model->service($id);

		echo json_encode($response, JSON_PRETTY_PRINT);
	}

	public function stock_laptop($id = 0)
	{
		$response 	= array(
			'result'	=> false,
			'msg'		=> ''
		...
		$response = $this->model->stock_laptop($id);
