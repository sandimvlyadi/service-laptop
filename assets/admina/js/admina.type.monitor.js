...
$('.li-master-type-monitor').addClass('active');
...
$(document).ready(function(){
	...
	table = $('#dataTable').DataTable({
        ...
        'ajax' : {
        	'url'	: baseurl + 'master-type-monitor/datatable/',
            'type'	: 'GET',
            'dataSrc' : function(response){
            	...
            	if (response.result) {
            		for(var x in response.data){
	            		...
	            		row.push({
	            			'no'            : i,
                            'nama_type'     : response.data[x].nama_type,
...
$('#dataTable').on('click', 'button[name="btn_edit"]', function(){
    ...
    $.ajax({
        type: 'GET',
        url: baseurl + 'master-type-monitor/edit/'+ id +'/',
        dataType: 'json',
        success: function(response){
            if(response.result){
                var d = response.data;

                $('input[name="nama_type"]').val(d.nama_type);
...
$('#dataTable').on('click', 'button[name="btn_delete"]', function(){
	...
	$.ajax({
        type: 'POST',
        url: baseurl + 'master-type-monitor/delete/',
        data: {
        	'id': id,
...
$('button[name="btn_save"]').click(function(){
    ...
    $.ajax({
        type: 'POST',
        url: baseurl + 'master-type-monitor/save/',
        data: {
        	'id': $(this).attr('id'),
